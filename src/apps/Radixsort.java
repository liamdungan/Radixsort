...
public class Radixsort {
	...
	throws IOException {
		...
		while (sc.hasNext()){
			...
			if (masterListRear == null){
				node = new Node<String>(thisLine, null);
				node.next = node;
			}
			else {
				node = new Node<String>(thisLine, null);
	...
	public void scatter(int pass) {
		...
		while ( current != masterListRear ) {
			
			current = ptr;
			ptr = ptr.next;	
	
			
			//turn character into a digit
			if(current.data.length() <= pass){		
				num = 0;
			}else{
				num = Character.digit(current.data.charAt( current.data.length() - pass - 1 ), radix) ;
			}	
			
			
			//place in proper buckets
			if( buckets[ num ] == null){
				
				buckets[ num ] = current;
				buckets[ num ].next = buckets[ num ];									
			}else{
				
				temp = buckets[ num ].next;
				buckets[ num ].next = current;
				current.next = temp;
				buckets[ num ] = current;			
			}
	...
	public void gather() {
		...
		while(p<radix && q<radix){
			...
			temp = buckets[q].next;
			buckets[q].next = buckets[p].next;
			buckets[p].next =  temp;
			
			masterListRear = buckets[p];
			q=p;
			p++;						
		}
